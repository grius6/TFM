...
```
├── TFM_preparacion_datos.ipynb     # Notebook 01 — Preparación de datos
├── TFM_modelado.ipynb              # Notebook 02 — Modelado
├── TFM_stock.ipynb                 # Notebook 03 — Impacto en stock y coste
├── anexo/
│   └── TFM_MLP.ipynb               # Exploración MLP (descartada del flujo principal)
├── data/
│   ├── raw/                        # Datos originales (no incluidos, ver nota)
│   ├── processed/                  # Datos procesados generados por el Notebook 01
│   └── results/                    # Resultados exportados por los Notebooks 02 y 03
├── data/external/
...
Los datos de ventas originales no están disponibles en este repositorio por razones de confidencialidad. Para reproducir el proyecto se necesitan los siguientes ficheros en `data/raw/`:

| Fichero | Descripción |
|--------|-------------|
| Ventas diarias por producto | Serie temporal de 731 días (22/03/2024 – 22/03/2026) con campos: producto, fecha, udsVenta, udsStock, bolOpen, bol_promo, bolHoliday |

Los ficheros `DatosCicloAprovisionamiento.xlsx` y `DatosPrecioMedio.xlsx`, facilitados por la empresa, están disponibles en `data/external/`.
...
Genera: `data/results/detalle_ahorro_por_producto.csv`, `data/results/resumen_impacto_stock.csv`
